...
describe("Japan RuleWatch service card", () => {
  it("publishes MCP discovery tags and English examples without claiming A2A", async () => {
    ...
    const card = await response.json() as { mcpEndpoint: string; links: { productPage: string; mcpRegistry: string }; skills: Array<{ tags: string[]; examples: string[]; firstTool: string }>; a2a: { supported: boolean } };
    expect(response.status).toBe(200);
    expect(card.mcpEndpoint).toBe("https://japan-rulewatch-mcp-mainnet.kadopi.workers.dev/mcp");
    expect(card.links.productPage).toContain("/products/japan-rulewatch");
    expect(card.links.mcpRegistry).toContain("io.github.kadopi%2Fjapan-rulewatch-mcp");
