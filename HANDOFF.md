...
- Official MCP Registry: `io.github.kadopi/japan-rulewatch-mcp` v0.1.0 was published via GitHub Actions OIDC on 2026-09-11. Registry status is `active` and `isLatest: true`; it points to the Mainnet Streamable HTTP endpoint.
- Registry workflow: `.github/workflows/publish-mcp-registry.yml` is manual-only (`workflow_dispatch`), uses GitHub OIDC, and publishes remote MCP metadata without npm publication or Worker deployment.
